...
cv::Mat volread(const cv::String &filename, int flags, int thresh) {
	...
	for (unsigned int plane = 0; plane < planenames.size(); plane++) {
		...
		if (thresh != -1) {
			threshold(tmp, tmp, thresh, 255, THRESH_BINARY);
		}
		if (plane == 0) {
...
bool volwrite(const cv::String& filename, const cv::Mat& volume, const std::vector<int>& params) {
	...
	for (unsigned i = 0; i < planes.size(); i++) {
		...
		if (!imwrite((vol_path / path(plane_name.str())).string(), planes[i], params))
			return false;
		os << plane_name.str() << '\n';
	}
	return true;
